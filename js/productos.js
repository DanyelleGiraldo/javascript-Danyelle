...
const cargarProductos= async()=>{
    try{
        const dbproductos= await fetch('http://localhost:3000/productos');

        if (!dbproductos.ok){
            throw new Error('error al cargar productos. Estado', dbproductos.status)
        }
        const productos=await dbproductos.json();
...
const guardorProducto = async (nuevoProducto)=>{
    try{
        ...
        if(!respuesta.ok){
            throw new Error('Error al crear producto. Estado: ',respuesta.status)
        }
